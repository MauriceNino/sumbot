...
export class ServerHandling {
  ...
  @command({
    name: "Version",
    aliases: ["version", "v"],
    description: "print the current version of this bot",
  })
  @logged
  public static async version(ctx: IActionContext) {
    ...
    await ctx.message.channel.send(
      `Current bot version: ${process.env.npm_package_version}`
    );
  }
  ...
  @command({
    ...
    description: "list all the commands of this bot",
  })
  public static async help(ctx: IActionContext) {
    ...
    const commandLength = commands.reduce((commandLength, command) => {
      ...
      return commandLength > mainCommandLength
        ? commandLength
        : mainCommandLength;
    }, 0);
    ...
    const lines = commands.map((command) => {
      ...
      return `  ${COMMAND_PREFIX} ${mainCommand}${" ".repeat(
        commandLength - mainCommand.length
      )}\t${desc}`;
    });
  ...
  public static async getMinecraftIp(ctx: IActionContext) {
    ...
    if (DASHBOARD_DOMAIN) {
      await ctx.message.channel.send(
        `Server Dashboard: <https://${DASHBOARD_DOMAIN}>`
      );
    } else {
